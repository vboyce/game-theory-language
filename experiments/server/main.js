...
function compareNumbers(a, b) //deal with the fact that js defaults to lexicographic sorting....
{
    return a - b;
}
...
function getType(game, repNum){
  ...
  if (game=="spikeMix"){
    ...
    if (spiked.includes(repNum)) return "spikeBoS"
      ...
      r=Math.random()
      if (r<.44) return "BoS" // distribution is (counting spikes), .5 BoS, .25 each PD variety
      else if (r<.72) return "easyPD" 
      else return "hardPD"
    }
